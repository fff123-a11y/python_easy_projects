...
class SingleLinkedList(object):
    ...
    def delete_by_tail(self, num):
        size = self.get_size()
        assert (num <= size) # 当num小于size的时候
        assert (num > 0) #链表长度要大于0
        pos = size - num
        count = 0
        self.point = self.head
        while count < size:
            ...
            self.point = self.point.next
            if count == pos:
                pointer = self.point.next #遍历到下一个结点
                self.point.next = self.point.next.next #遍历到下下个结点
                del pointer
...
class StudentControlSystem(SingleLinkedList):
    ...
    def search_info(self):
        ...
        # 把信息以下列形式打印出来
        print('ID 姓名 成绩')
        print('{} {} {}'.format(data['id'], data['name'], data['grade']))  # 格式化输出
